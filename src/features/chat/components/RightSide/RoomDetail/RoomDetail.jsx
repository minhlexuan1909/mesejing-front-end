...
import { getUsersInRoomThunk, setNameOfRoomThunk,
  deleteUserInRoomThunk, leaveRoomThunk,setAvatarOfRoomThunk
} from "../../../services/room/roomThunk";
import { tokenSelector } from "../../../../auth/services/authSlice";
import { usersInRommSelector, selectedRoomSelector } from "../../../services/room/roomSlice";
...
const RoomDetail = ({roomDetail}) => {
  ...
  const handleCfRoomSettings = async () => {
    ...
    dispatch(setNameOfRoomThunk({roomID: roomID, name: nameOfRoomInput, token: token}))
  ...
  const deleteUserRoom = (deleteUserID) => {
    dispatch(deleteUserInRoomThunk({roomID: roomID, deleteUserID: deleteUserID, token: token}))
  ...
  useEffect(() => {
    dispatch(getUsersInRoomThunk({ roomID: roomID, token }));
    setPreviewImgRoom(`${process.env.REACT_APP_BASE_IMAGE_URL}${roomDetail.avatarGroup.replaceAll(" ", "-")}`)
        ...
        <div className="chat_inf-wrapper">
          ...
          <div className="user_inf">
            ...
            <div className="user_name">
              ...
              <FontAwesomeIcon icon={faPenToSquare} onClick={() => setIsOpenModalRoomSettings(true)}/>
              }
            </div>
          ...
          <div className="chatSettings">
            <div className="chatSettings-item" onClick={()=> {
              setMODE_VIEW(3);
            }}>
          ...
          <div className="other-tools">
            ...
            <div className="tools-list_items">
              ...
              <div className="tools-item" onClick={handleLeaveRoom}>
                ...
                <span style={{ color: "#db342e" }}>Rời cuộc trò chuyện</span>
              </div>
            </div>
          </div>
          ...
          >
            <div className="modal-settings-container">
              ...
              <span>Bạn có chắn muốn đổi thông tin nhóm, khi xác nhận thông tin nhóm mới sẽ hiển thị với tất cả thành viên.</span>
              <div className="input-name-room">
                <Input placeholder="Nhập tên nhóm" value={nameOfRoomInput} onChange={(e) => setNameOfRoomInput(e.target.value)}/>
              </div>
            </div>
          </Modal>
        </div>
            ...
            {usersInRoom.map((item) => {
              ...
              <div className="userItem">
                <div className="user-inf">
                  <img
                    src={`${process.env.REACT_APP_BASE_IMAGE_URL}${item.user.avatar.replaceAll(" ", "-")}`}
                    alt="User"
                  ></img>
                  <div className="user_name">
                    ...
                    <Menu>
                      <Menu.Item
                        ...
                        onClick={() => {
                          ...
                          dispatch(userProfileAction.setSelectedUser(item.user._id));
                        }}
                      >
                        <FontAwesomeIcon icon={faAddressCard} style={{"margin-right": "8px"}}/>
                        Xem profile 
                      </Menu.Item>
                      <Menu.Divider />
                      {/* <Menu.Item key="2">
